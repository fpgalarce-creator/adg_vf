    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 bg-[color:var(--bg-primary)]/95 backdrop-blur-md transition-all duration-500 ${scrolled ? 'shadow-lg shadow-black/5 border-b border-[color:var(--border-soft)]' : 'shadow-sm'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <div className="hidden md:flex items-center gap-1">
            ...
            <Link to="/productos#destacados" onClick={() => setMobileOpen(false)} className={`${linkBase} text-[color:var(--text-secondary)] hover:text-[color:var(--text-primary)] hover:bg-[color:var(--bg-secondary)]`}>Destacados</Link>
      ...
      <div className={`md:hidden transition-all duration-300 overflow-hidden ${mobileOpen ? 'max-h-80 opacity-100' : 'max-h-0 opacity-0'}`}>
        <div className="bg-[color:var(--bg-card)] border-t border-[color:var(--border-soft)] px-4 py-4 space-y-1">
          ...
          <Link to="/productos#destacados" onClick={() => setMobileOpen(false)} className="block w-full text-left px-4 py-3 rounded-xl text-[color:var(--text-secondary)] hover:bg-[color:var(--bg-secondary)] hover:text-[color:var(--text-primary)] font-medium">Destacados</Link>
