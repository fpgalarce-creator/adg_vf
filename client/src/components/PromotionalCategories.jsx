        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5 lg:gap-6">
          {promoCategories.map((category, index) => (
            <ScrollAnimation key={category.title} delay={index * 80}>
              <Link
                to={category.href}
                className="group relative isolate overflow-hidden rounded-[1.55rem] sm:rounded-[1.8rem] min-h-[220px] sm:min-h-[255px] lg:min-h-[285px] block border border-white/65 shadow-[0_16px_32px_rgba(30,34,23,0.12)] hover:shadow-[0_22px_40px_rgba(30,34,23,0.18)] transition-all duration-500 hover:-translate-y-1"
              ...
              >
                ...
                <div className="relative z-10 h-full flex flex-col justify-end p-6 sm:p-7 lg:p-8">
                  <div className="max-w-[35ch] rounded-2xl bg-[#f8f4ea]/84 border border-white/45 px-4 py-3.5 backdrop-blur-[2.5px] shadow-[0_8px_18px_rgba(17,22,14,0.1)]">
                    <p className="text-[color:var(--primary)] text-[0.68rem] sm:text-xs uppercase tracking-[0.16em] mb-1.5 font-semibold">Categoría destacada</p>
                    <h3 className="text-[color:var(--text-primary)] font-heading text-[1.45rem] sm:text-[1.6rem] leading-tight font-semibold text-balance">{category.title}</h3>
                    <p className="mt-1.5 text-[color:var(--text-secondary)] text-sm sm:text-[0.95rem]">{category.description}</p>

                    <span className="mt-3 inline-flex items-center gap-2 text-[color:var(--primary)] font-semibold text-sm sm:text-[0.95rem]">
                      Ver productos
